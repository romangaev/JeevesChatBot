...
def call_back_buttons(bot, update):
    ...
    if update.callback_query.data == "Pronunciation":
        ...
        if attachment == "" or attachment =="Couldn't find any audio":
            bot.sendMessage(update.callback_query.message.chat_id, text="К сожалению, у меня не нашлось ни одной аудиозаписи...")
        else:
            url = attachment
            print(url)
...
def main():
    ...
    job.run_daily(callback_daily, time(21,10), context=None, name=None)
